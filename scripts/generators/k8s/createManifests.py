...
def main():
    ...
    if isinstance(yaml_data, dict):
        ...
        gconfig = {
            key: globalConfig[key] for key in keys_to_keep if key in globalConfig
        }
        debug_print("gconfig: gconfig")
            ...
            if key == "services":
                ...
                for service, config in value.items():
                    ...
                    if config["type"] in application_services:
                        ...
                        print(
                            f"create Appplication Service of type {config['type']} named {service}"
                        )
                        write_yaml(
                            f"./deployments/{service}-configmap.yaml",
                            renderConfigMap2(key, config, service),
                        )
                        write_yaml(
                            f"./deployments/{service}-deployment.yaml",
                            renderDeployment2(key, config, service),
                        )
                        if config.get("exposedPort", None) != None:
                            debug_print(f"ExposedPort: {config.get('exposedPort')}")
                            write_yaml(
                                f"./deployments/{service}-service-ext.yaml",
                                renderService2(key, config, service),
                            )
                            config2 = config
                            ...
                            write_yaml(
                                f"./deployments/{service}-service.yaml",
                                renderService2(key, config, service),
                            )
                        else:
                            write_yaml(
                        ...
                        print(
                            f"Unsupported service type detected {config['type']} named {service}"
                        )
            elif key == "loaders":
                for loader, config in value.items():
                    if config["type"] in loader_services:
                        print(
                            f"create loader service of type {config['type']} named {loader}"
                        )
                        ...
                        write_yaml(
                            f"./deployments/{loader}-configmap.yaml",
                            renderConfigMap2(key, config, loader),
                        )
                        write_yaml(
                            f"./deployments/{loader}-deployment.yaml",
                            renderDeployment2(key, context, loader),
                        )
                        # There is no need to create a service for a loadgenerator at this point in time
                    else:
                        print(
                            f"Unsupported loader type detected {config['type']} named {loader}"
                        )
            elif key == "databases":
                for service, config in value.items():
                    config["agent"] = False
                    if config["type"] in db_services:
                        config = merge_dicts(globalConfig, config)
                        print(
                            f"create DB Service of type {config['type']} named {service}"
                        )
                        write_yaml(
                            f"./deployments/{service}-configmap.yaml",
                            renderConfigMap2(key, config, service),
                ...
                #
                print(
                    f"found unsupported key {key} in config - try rendering deployment, service and configmap"
                )
                try:
                    for service, config in value.items():
                        config = merge_dicts(globalConfig, config)
